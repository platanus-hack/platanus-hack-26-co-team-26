...
import co.sismomesh.core.application.ports.IdentityPort
import co.sismomesh.core.platform.secureRandomBytes
...
object PseudonymDeriver {
    fun derive(identitySeed: ByteArray, disasterId: String, epoch: Long): ByteArray {
        val info = "sismomesh-pseudonym-v1:$disasterId:$epoch".encodeToByteArray()
...
class X25519Handshake {
    ...
    fun deriveSessionKey(sharedSecret: ByteArray, transcript: ByteArray): ByteArray =
        Hkdf.deriveKey(ikm = sharedSecret, salt = transcript, info = "sismomesh-session-v1".encodeToByteArray(), length = 32)

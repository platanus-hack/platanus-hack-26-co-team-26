...
import co.sismomesh.core.domain.vo.PeerId
import co.sismomesh.core.domain.vo.PeerLink
import co.sismomesh.core.dtn.InventoryBloom
import co.sismomesh.core.dtn.outboxSnapshot
import co.sismomesh.core.protocol.BundleWireCodec.toDomainBundle
import co.sismomesh.core.protocol.BundleWireCodec.toWire
...
class BleGattClient(
    ...
    private inner class Session(
        ...
        private suspend fun enableNotificationsThenSync(gatt: BluetoothGatt) {
            val service = gatt.getService(BleGattProfile.SERVICE_UUID) ?: return finishWithError(gatt, "Servicio SismoMesh no encontrado en el peer")
            ...
            if (inventoryChar == null || bundleChar == null) return finishWithError(gatt, "Características SismoMesh incompletas en el peer")
